...
static int xadc_probe(struct platform_device *pdev)
{
    ...
    printk("we are probing for xlinx xadc \n");
...
static int __init xadc_driver_init(void)
{
    printk(KERN_INFO " INIT FUNCTION IN XADC ...\n");
    return platform_driver_register(&xadc_driver);
}

static void __exit xadc_driver_exit(void)
{
    printk(KERN_INFO "exiting AXI module ...\n");
    platform_driver_unregister(&xadc_driver);
}

module_init(xadc_driver_init);
module_exit(xadc_driver_exit);
