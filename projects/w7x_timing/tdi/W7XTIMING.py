...
class w7x_timing_lib(object):
    ...
    @staticmethod
    def _byref(*args):
        return tuple((w7x_timing_lib._NULL if a is None else _c.byref(a)) for a in args)
    def __init__(self):
        try:
            ...
            print('w7x_timing_lib: '+ str(exc))
    def arm(self):
        self.cdll.arm()
    def disarm(self):
        self.cdll.disarm()
    def makeClock(self,delay=None,width=None,period=None,cycle=None,repeat=None,count=None):
        args = w7x_timing_lib._toctypes(delay,width,period,cycle,repeat,count)
        delay,width,period,cycle,repeat,count = w7x_timing_lib._byref(*args)
    ...
    def makeSequence(self,delay=None,width=None,period=None,cycle=None,repeat=None,times=[]):
        ...
        args  = w7x_timing_lib._toctypes(delay,width,period,cycle,repeat,len(times))
        delay,width,period,cycle,repeat,count = w7x_timing_lib._byref(*args)
        self.cdll.makeSequence(delay,width,period,cycle,repeat,count,timref)
    def reinit(self,delay=6e7):
        delay = None if delay  is None else _c.c_uint64(int(delay))
        ref   = w7x_timing_lib._NULL if delay is None else _c.byref(delay)
    ...
    def gate(self,value=0):
        value = _c.c_uint8(w7x_timing_lib._tobyte(value))
...
class remote(object):
    ...
    def extclk(self,value=True):
        ...
        return self._exchange(msg)
    def gate(self,val=0):
        msg = remote._makeMsg('G','B',1,w7x_timing_lib._tobyte(val))
        return self._exchange(msg)
    def invert(self,val=0):
        msg = remote._makeMsg('I','B',1,w7x_timing_lib._tobyte(val))
        return self._exchange(msg)
